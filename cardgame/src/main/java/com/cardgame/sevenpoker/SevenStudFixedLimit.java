...
public class SevenStudFixedLimit implements Game {
    ...
    // 서버용 자동 1판: 콘솔 입력 없이 기존 로직 재사용(잔액=stackForHand)
    public int playOneRoundForBet(com.cardgame.card.Deck deck, int ante, int stackForHand) {
        // 기존에 playOneRoundForBet(Scanner, Deck, int ante, int stack) 이 있다면 그걸 호출
        java.util.Scanner dummy = new java.util.Scanner(new java.io.ByteArrayInputStream(new byte[0]));
        return playOneRoundForBet(dummy, deck, ante, stackForHand);
    }
}
