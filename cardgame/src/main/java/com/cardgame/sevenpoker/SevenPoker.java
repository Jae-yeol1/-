...
public class SevenPoker implements Game {
    ...
    // SevenPoker.java 안에 추가
    public int playOneRoundForBet(com.cardgame.card.Deck deck, int ante) {
        if (ante < 1) ante = 1;
        int pot = ante * 2; // 플레이어 ante + 딜러 매칭

        java.util.List<com.cardgame.card.Card> p = new java.util.ArrayList<>(7);
        java.util.List<com.cardgame.card.Card> d = new java.util.ArrayList<>(7);

        // 7장씩 배분 (3다운+4업 개념은 표시용에서만 의미, 자동판은 단순 7장)
        for (int i = 0; i < 7; i++) { p.add(deck.draw()); d.add(deck.draw()); }
        ...
        int cmp = ps.compareTo(ds);
        if (cmp > 0) return +ante;   // 팟(2*ante) - 본인 ante(=순이익 +ante)
        if (cmp < 0) return -ante;   // 패배 시 순손실 -ante
        return 0;                    // 무승부
    }
