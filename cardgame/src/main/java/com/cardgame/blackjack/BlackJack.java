...
public class BlackJack implements Game {
    ...
    public int playOneRoundForBet(com.cardgame.card.Deck deck, int bet) {
        java.util.List<com.cardgame.card.Card> player = new java.util.ArrayList<>();
        java.util.List<com.cardgame.card.Card> dealer = new java.util.ArrayList<>();

        // 초기 2장씩
        player.add(deck.draw()); player.add(deck.draw());
        dealer.add(deck.draw()); dealer.add(deck.draw());

        // 점수 계산 (ACE=11, 21 초과 시 ACE→1 보정)
        java.util.function.ToIntFunction<java.util.List<com.cardgame.card.Card>> value = hand -> {
            int sum = 0, aces = 0;
            for (var c : hand) {
                switch (c.rank()) {
                    case ACE -> { sum += 11; aces++; }
        ...
        // 플레이어: 17 미만이면 히트
        while (value.applyAsInt(player) < 17) player.add(deck.draw());
        int pv = value.applyAsInt(player);
        if (pv > 21) return -bet; // 버스트

        // 딜러: S17
        while (value.applyAsInt(dealer) < 17) dealer.add(deck.draw());
        int dv = value.applyAsInt(dealer);

        if (dv > 21) return +bet;     // 딜러 버스트
        if (pv > dv)  return +bet;    // 승
        if (pv < dv)  return -bet;    // 패
        return 0;                     // 푸시
    }
}
