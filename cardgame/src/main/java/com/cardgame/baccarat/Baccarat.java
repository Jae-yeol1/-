...
public class Baccarat implements Game {
    ...
    // Baccarat.java 안에 추가
    public int playOneRoundForBet(com.cardgame.card.Deck deck, int bet, Bet on) {
        java.util.List<com.cardgame.card.Card> p = new java.util.ArrayList<>();
        java.util.List<com.cardgame.card.Card> b = new java.util.ArrayList<>();

        // 초기 2장씩
        p.add(deck.draw()); p.add(deck.draw());
        b.add(deck.draw()); b.add(deck.draw());

        java.util.function.ToIntFunction<com.cardgame.card.Card> vCard = c -> switch (c.rank()) {
            case ACE -> 1;
            case TWO -> 2; case THREE -> 3; case FOUR -> 4; case FIVE -> 5;
            case SIX -> 6; case SEVEN -> 7; case EIGHT -> 8; case NINE -> 9;
            default -> 0; // TEN, JACK, QUEEN, KING
        };
        java.util.function.ToIntFunction<java.util.List<com.cardgame.card.Card>> total = hand -> {
            int s = 0; for (var c : hand) s += vCard.applyAsInt(c); return s % 10;
        };

        int pt = total.applyAsInt(p), bt = total.applyAsInt(b);

        // Natural
        if (pt >= 8 || bt >= 8) {
            return settleBaccarat(pt, bt, bet, on);
        }

        // Player 3rd
        com.cardgame.card.Card p3 = null;
        boolean playerDraws = (pt <= 5);
        if (playerDraws) {
            p3 = deck.draw();
            p.add(p3);
            pt = total.applyAsInt(p);
        }

        // Banker 3rd (플레이어 3rd 유무 별 규칙)
        if (!playerDraws) {
            // Player 스탠드 → Banker 0~5 드로우, 6/7 스탠드
            if (bt <= 5) { b.add(deck.draw()); bt = total.applyAsInt(b); }
        } else {
            int pv3 = vCard.applyAsInt(p3);
            // 표준 테이블
            if (bt <= 2) {
                b.add(deck.draw());
            } else if (bt == 3) {
                if (pv3 != 8) b.add(deck.draw());
            } else if (bt == 4) {
                if (pv3 >= 2 && pv3 <= 7) b.add(deck.draw());
            } else if (bt == 5) {
                if (pv3 >= 4 && pv3 <= 7) b.add(deck.draw());
            } else if (bt == 6) {
                if (pv3 == 6 || pv3 == 7) b.add(deck.draw());
            } // 7 스탠드
            bt = total.applyAsInt(b);
        }

        return settleBaccarat(pt, bt, bet, on);
    }

    // 같은 클래스 안에 헬퍼 하나 더 (정산)
    private int settleBaccarat(int pt, int bt, int bet, Bet on) {
        // Player/Banker에 베팅했는데 TIE면 푸시
        if (on == Bet.PLAYER) {
            if (pt > bt) return +bet;             // 1:1
            if (pt < bt) return -bet;
            return 0;                              // tie push
        } else if (on == Bet.BANKER) {
            if (bt > pt) {
                // 1:1 - 5% 커미션
                int win = bet;
                int comm = (int)Math.round(win * 0.05);
                return win - comm;
            }
            if (bt < pt) return -bet;
            return 0;                              // tie push
        } else { // TIE
            if (pt == bt) return bet * 8;          // 8:1
            return -bet;
        }
    }
